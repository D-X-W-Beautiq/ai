...
NIA → Feedback(옵션) → Product → Style → Makeup → Customization
...
# NIA 응답에 feedback이 포함되므로, 별도 Feedback API 호출은 기본적으로 비활성화
USE_FEEDBACK_API = False
...
def _print_time_summary(total_key: str = "TOTAL") -> None:
    ...
    print("실행 시간 요약")
    print("="*60)
    ordered: List[str] = ["NIA"]
    if USE_FEEDBACK_API:
        ordered.append("Feedback")
    ordered += ["Product", "Style", "Makeup", "Customization"]
    for k in ordered:
        if k in _TIMES:
            _print_time(k)
...
def validate_nia_response(resp: Dict[str, Any]) -> List[str]:
    """NIA 응답: predictions + feedback 통합 스펙 검증"""
    e: List[str] = []
    if "status" not in resp:
        e.append("❌ 필수 필드 누락: status")
        return e
    ...
    if resp["status"] == "success":
        if "predictions" not in resp:
            ...
            for k in ["moisture_reg","elasticity_reg","wrinkle_reg","pigmentation_reg","pore_reg"]:
                if k not in pred:
                    e.append(f"❌ predictions.{k} 누락")
                elif not isinstance(pred[k], int):
                    e.append(f"❌ predictions.{k} 타입오류:{type(pred[k]).__name__}")
                elif not (0 <= pred[k] <= 100):
                    e.append(f"❌ predictions.{k} 범위오류:{pred[k]}")
...
def validate_product_response(resp: Dict[str, Any]) -> List[str]:
    ...
    if resp["status"] == "success":
        ...
        if not isinstance(recs, list):
            e.append("❌ recommendations 누락/타입 오류")
        else:
            for i, r in enumerate(recs):
                if not isinstance(r.get("product_id"), str):
                    e.append(f"❌ rec[{i}].product_id 누락/타입 오류")
                if not isinstance(r.get("reason"), str):
                    e.append(f"❌ rec[{i}].reason 누락/타입 오류")
    elif resp["status"] == "error":
        if "message" not in resp:
            e.append("❌ 필수 필드 누락: message")
        if "error_code" not in resp:
            e.append("❌ 필수 필드 누락: error_code")
    return e
...
def validate_style_response(resp: Dict[str, Any]) -> List[str]:
    ...
    if resp["status"] == "success":
        ...
        if not isinstance(results, list):
            e.append("❌ results 누락/타입 오류")
        else:
            for i, r in enumerate(results):
                if not isinstance(r.get("style_id"), str):
                    e.append(f"❌ results[{i}].style_id 누락/타입 오류")
                if not isinstance(r.get("style_image_base64"), str):
                    e.append(f"❌ results[{i}].style_image_base64 누락/타입 오류")
    elif resp["status"] == "error" and "message" not in resp:
        e.append("❌ 필수 필드 누락: message")
    return e
...
def validate_makeup_response(resp: Dict[str, Any]) -> List[str]:
    ...
    if "status" not in resp:
        ...
        return e
    if resp["status"] == "success":
        if not isinstance(resp.get("result_image_base64"), str):
            e.append("❌ result_image_base64 누락/타입 오류")
    elif resp["status"] == "error" and "message" not in resp:
        e.append("❌ 필수 필드 누락: message")
    return e
...
def step2_feedback(predictions_path: Path):
    """(옵션) 별도 Feedback API 테스트용. 기본적으로는 사용하지 않음."""
    print("\n" + "="*60)
...
def main():
    ...
    if USE_FEEDBACK_API:
        ...
        print("\n[Feedback] 스킵: NIA 응답의 feedback 필드를 사용합니다.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_feedback_api", action="store_true",
                        help="별도 Feedback API(/feedback/generate) 호출을 활성화합니다.")
    ...
    # CLI 옵션으로 Feedback API 사용 여부 제어
    if args.use_feedback_api:
        USE_FEEDBACK_API = True
